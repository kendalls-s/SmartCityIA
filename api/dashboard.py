...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab1:
        c1,c2,c3,c4 = st.columns(4)
        for col,(lbl,val,badge) in zip([c1,c2,c3,c4],[("Accuracy","80%","badge-green"),("Val Loss","0.4252","badge-blue"),("Epochs","6*","badge-violet"),("Clases","4","badge-amber")]):
            with col:
        ...
        st.markdown("<div style='font-size:0.75rem;color:#475569;margin-bottom:1rem'>* EarlyStopping activado en epoch 6 (patience=2) · Mejor val_loss en epoch 4</div>", unsafe_allow_html=True)
        cl, cr = st.columns(2)
        with cl:
            st.markdown("<div class='section-card'><h3>Loss y Accuracy durante entrenamiento</h3>", unsafe_allow_html=True)
            epochs   = [1,2,3,4,5,6]
            loss     = [0.9110,0.5771,0.4756,0.4226,0.3808,0.3255]
            val_loss = [0.6628,0.4487,0.5161,0.4252,0.5940,0.5279]
            acc      = [0.6084,0.7739,0.8238,0.8344,0.8594,0.8807]
            val_acc  = [0.7547,0.8246,0.7910,0.8246,0.7444,0.7826]
            fig, axes = dark_fig_multi(1, 2, w=11, h=4)
        ...
        with cr:
            ...
            cm = np.array([[51,58,2,36],[12,561,0,0],[0,0,166,5],[49,26,26,80]])
        ...
        df_rep = pd.DataFrame({'Clase':['Asaltos','Normal','Pelea','Vandalismo','Macro avg','Weighted avg'],'Precision':[0.46,0.87,0.86,0.66,0.71,0.78],'Recall':[0.35,0.98,0.97,0.44,0.68,0.80],'F1-Score':[0.39,0.92,0.91,0.53,0.69,0.78],'Support':[147,573,171,181,1072,1072]})
